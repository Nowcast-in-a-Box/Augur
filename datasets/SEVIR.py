...
from dataset.SEVIR_dataloader import SEVIRDataLoader
from utils.augmentation import TransformsFixRotation
...
class SEVIRTorchDataset(TorchDataset):
    ...
    def __getitem__(self, index):
        ...
        data = data[1::2, :, :]
        half_time = int(data.shape[0] / 2)
        return {
            "sequence": data[:-half_time, :, :],
            "target": data[half_time:, :, :]
        }
...
class SEVIRLightningDataModule(LightningDataModule):
    ...
    def prepare_data(self) -> None:
        if os.path.exists(self.sevir_dir):
            ...
            # else:
            #     raise NotImplementedError
            raise FileNotFoundError("SEVIR dataset not found! ")
