...
def compute_mask(D, H, W, window_size, shift_size, device):
    ...
    attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(
        2
    )  # broadcasting: (nW, 1, ws[0]*ws[1]*ws[2]) - (nW, ws[0]*ws[1]*ws[2], 1)
...
class BasicLayer_skip(nn.Module):
    ...
    def __init__(
        ...
        if self.subsample is not None:
            self.subsample = subsample(
                dim=dim, norm_layer=norm_layer, scale=subsample_scale
            )
...
class CubicDualUpsample(nn.Module):
    def __init__(
        ...
        self.conv_p1 = nn.Conv3d(
            dim,
            int(self.scale_factor / 2) * dim,
            kernel_size,
            stride_size,
            padding,
            bias=False,
        ...
        self.conv_p2 = nn.Conv3d(
            dim // 2, dim // 2, kernel_size, stride_size, padding, bias=False
        )
        ...
        self.conv_b2 = nn.Conv3d(
            dim, dim // 2, kernel_size, stride_size, padding, bias=False
        )

        self.conv_merge = nn.Conv3d(
            dim, dim // 2, kernel_size, stride_size, padding, bias=False
        )
...
class exPreCast(LightningModule):
    def __init__(
        self,
        input_frames=7,
        output_frames=6,
        ...
        self.encoder_time_dims = [
            (self.input_frames + 1) // 2 * self.downsampling_scale[0] ** (i + 1)
            for i in range(self.num_layers - 1)
        ]

        if self.skip_connection == "concat":
            self.decoder_time_dims = [
                self.encoder_time_dims[-1] * self.upsampling_scale[0]
                + self.encoder_time_dims[-1]
            ]
            for i in range(self.num_layers - 2):
                self.decoder_time_dims.append(
                    self.decoder_time_dims[-1] * self.upsampling_scale[0]
                    + self.encoder_time_dims[-(i + 1)]
                )
        ...
        self.bottleneck_upscale = CubicDualUpsample(
            dim=self.bottleneck_features, scale=self.upsampling_scale
        )
        ...
        self.patch_expand3d = PatchExpanding3D(
            patch_size=self.patch_expan_size,
    ...
    def load_pretrained(self, pretrained_path):
        pretrained_dict = torch.load(pretrained_path, weights_only=False)["state_dict"]
    ...
    def training_step(self, batch):
        ...
        sequence = batch["sequence"].unsqueeze(1)
        target = batch["target"]
        
        pred = self.forward(sequence).squeeze(1)
    ...
    def configure_optimizers(self):
        ...
        self.loss = FACL(self.trainer.estimated_stepping_batches)
