...
class Model(object):
    def __init__(self, configs):
        ...
        self.scheduler = lr_scheduler.ExponentialLR(
            self.optimizer, gamma=configs.lr_decay
        )
    ...
    def load(self, pm_checkpoint_path):
        ...
        stats = torch.load(
            pm_checkpoint_path, map_location=torch.device(self.configs.device)
        )
    ...
    def train(self, data, mask, itr):
        ...
        if (
            itr >= self.configs.sampling_stop_iter
            and itr % self.configs.delay_interval == 0
        ):
            self.scheduler.step()
        ...
        return (
            next_frames,
            loss_l1.detach().cpu().numpy(),
            loss_l2.detach().cpu().numpy(),
        )
