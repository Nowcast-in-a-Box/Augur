...
from models.MAU.MAUCell import MAUCell
import math
    ...
    def forward(self, frames, mask_true):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for i in range(self.tau):
                tmp_t.append(
                    torch.zeros([batch_size, in_channel, height, width], device=frames.device)
                )
                tmp_s.append(
                    torch.zeros([batch_size, in_channel, height, width], device=frames.device)
                )
        ...
        for t in range(self.configs.total_length - 1):
            ...
            if t == 0:
                for i in range(self.num_layers):
                    zeros = torch.zeros(
                        [batch_size, self.num_hidden[i], height, width], device=frames.device
                    )
                    T_t.append(zeros)
            S_t = frames_feature
            for i in range(self.num_layers):
