...
class AmpTimeCell(nn.Module):
    def __init__(self, t_in, t_out, size_factor=1):
        ...
        self.b1 = nn.Parameter(
            (self.scale * torch.randn(2, 1, 1, 1, t_out * size_factor))
        )
        self.w2 = nn.Parameter(
            (self.scale * torch.randn(2, t_out * size_factor, t_out))
        )
    ...
    def forward(self, x):
        ...
        x1_real = (
            torch.einsum("bchwt,to->bchwo", xf.real, self.w1[0])
            - torch.einsum("bchwt,to->bchwo", xf.imag, self.w1[1])
            + self.b1[0]
        )
        x1_imag = (
            torch.einsum("bchwt,to->bchwo", xf.real, self.w1[1])
        ...
        x2_real = (
            torch.einsum("bchwt,to->bchwo", x1_real, self.w2[0])
            - torch.einsum("bchwt,to->bchwo", x1_imag, self.w2[1])
            + self.b2[0]
        )
        x2_imag = (
            torch.einsum("bchwt,to->bchwo", x1_real, self.w2[1])
...
class AmpliNet(nn.Module):
    def __init__(
        self, pre_seq_length, aft_seq_length, dim, hidden_dim, n_layers=3, mlp_ratio=2
    ):
        super().__init__()
        ...
        self.amplist = nn.ModuleList(
            [
                AmpCell(
                    pre_seq_length if i == 0 else aft_seq_length,
                    aft_seq_length,
                    hidden_dim,
                )
...
class PhaseNet(nn.Module):
    def __init__(
        ...
        input_shape = (h, w // 2 + 1)
        self.pre_seq_length, self.aft_seq_length = pre_seq_length, aft_seq_length
        self.pha_conv0 = nn.Conv2d(
            2 + input_dim * pre_seq_length, input_dim * aft_seq_length, 1
        )
        self.phase_0 = nn.Sequential(
            ResnetBlock(2 + input_dim * pre_seq_length, hidden_dim, kernel_size=1),
        ...
        self.pha_conv1 = nn.Conv2d(
            4 * input_dim * aft_seq_length, input_dim * aft_seq_length, 1
        )
    ...
    def forward(self, x):  # x:[b,t,c,h,w]
        ...
        x_phas_t = rearrange(
            x_phas_t, "b (t c) h w -> b t c h w", t=self.aft_seq_length
        )
...
class AlphaMixer(nn.Module):
    def __init__(
        self, input_shape, spec_num, input_dim, hidden_dim, aft_seq_length
    ) -> None:
        super().__init__()
        h, w = input_shape
        ...
        self.out_mixer = nn.Sequential(
            ResnetBlock(3 * input_dim, hidden_dim),
            ResnetBlock(hidden_dim, hidden_dim),
...
class AlphaPre(nn.Module):
    def __init__(
        ...
        self.alphamixer = AlphaMixer(
            input_shape, spec_num, input_dim, hidden_dim, aft_seq_length
        )
    ...
    def forward(self, x, y, compute_loss):  # x:[b,t,c,h,w]
        self.itr += 1
        xas = self.amplinet(x)
    ...
    def predict(self, frames_in, frames_gt=None, compute_loss=False):
        ...
        if compute_loss:
            ...
            pha_loss = (
                1 - torch.cos(frames_pha * self.spec_mask - x_phas_t * self.spec_mask)
            ).sum() / (self.spec_mask.sum() * B * self.aft_seq_length * self.input_dim)
...
class ResnetBlock(nn.Module):
    def __init__(
        self, dim, dim_out, groups=8, kernel_size=3, padding_mode="zeros"
