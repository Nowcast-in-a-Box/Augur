...
from models.EarthFarseer.Global_Fourier_Transformer import GF_Block

from models.EarthFarseer.Local_CNN_Branch import *
...
class FoTF(nn.Module):
    ...
    def forward(self, x_raw):
        ...
        for _ in range(self.num_interactions):
            gf_features_up = self.up(gf_features.view(B * T, C, H, W)).view(
                B, T, C, H, W
            )
            lc_features = self.conv1x1(lc_features.view(B * T, C, H, W)).view(
                B, T, C, H, W
            )
            ...
            gf_features_down = self.down(gf_features.view(B * T, C, H, W)).view(
                B, T, C, H, W
            )
            lc_features = self.conv1x1(lc_features.view(B * T, C, H, W)).view(
                B, T, C, H, W
