...
class ConvolutionalNetwork(nn.Module):
    ...
    class ConvSC(nn.Module):
        def __init__(self, C_in, C_out, stride, transpose=False, act_norm=True):
            ...
            if stride == 1:
                transpose = False
            self.conv = ConvolutionalNetwork.BasicConv2d(
                C_in,
                C_out,
                kernel_size=3,
                stride=stride,
    ...
    class Inception(nn.Module):
        def __init__(self, C_in, C_hid, C_out, incep_ker=[3, 5, 7, 11], groups=8):
            ...
            for ker in incep_ker:
                layers.append(
                    ConvolutionalNetwork.GroupConv2d(
                        C_hid,
                        C_out,
                        kernel_size=ker,
                        stride=1,
    ...
    class Encoder(nn.Module):
        def __init__(self, C_in, C_hid, N_S):
            ...
            self.enc = nn.Sequential(
                ...
                *[
                    ConvolutionalNetwork.ConvSC(C_hid, C_hid, stride=s)
                    for s in strides[1:]
                ]
            )
    ...
    class Decoder(nn.Module):
        def __init__(self, C_hid, C_out, N_S):
            ...
            self.dec = nn.Sequential(
                *[
                    ConvolutionalNetwork.ConvSC(C_hid, C_hid, stride=s, transpose=True)
                    for s in strides[:-1]
                ],
                ConvolutionalNetwork.ConvSC(
                    2 * C_hid, C_hid, stride=strides[-1], transpose=True
                )
            )
    ...
    class Mid_Xnet(nn.Module):
        def __init__(
            self, channel_in, channel_hid, N_T, incep_ker=[3, 5, 7, 11], groups=8
        ):
            super(ConvolutionalNetwork.Mid_Xnet, self).__init__()

            self.N_T = N_T
            enc_layers = [
                ConvolutionalNetwork.Inception(
                    channel_in,
                    channel_hid // 2,
                    channel_hid,
                    incep_ker=incep_ker,
    ...
    class skip_connection(nn.Module):
        def __init__(
            ...
            self.hid = ConvolutionalNetwork.Mid_Xnet(
                T * hid_S, hid_T, N_T, incep_ker, groups
            )
