...
class PhyCell(LightningModule):
    ...
    def initHidden(self,batch_size):
        ...
        for i in range(self.n_layers):
            self.H.append( torch.zeros(batch_size, self.input_dim, self.input_shape[0], self.input_shape[1], device=self.device) )
...
class ConvLSTM(LightningModule):
    ...
    def initHidden(self,batch_size):
        self.H, self.C = [],[]  
        for i in range(self.n_layers):
            self.H.append( torch.zeros(batch_size,self.hidden_dims[i], self.input_shape[0], self.input_shape[1], device=self.device) )
            self.C.append( torch.zeros(batch_size,self.hidden_dims[i], self.input_shape[0], self.input_shape[1], device=self.device) )
...
class EncoderRNN(LightningModule):
    def __init__(self, phycell, convcell, in_channels=1, out_channels=1):
        super(EncoderRNN, self).__init__()
        self.encoder_E = encoder_E(nc=in_channels)  # general encoder with configurable input channels
        ...
        self.decoder_D = decoder_D(nc=out_channels) # general decoder with configurable output channels 
