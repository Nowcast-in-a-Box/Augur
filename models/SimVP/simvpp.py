'''
Modified SimVP 
To support non-equal input and output length

This file is a combination of multiple source files in the repository:
...
class Mid_Xnet(nn.Module):
    def __init__(self, hid_S, hid_T, T, T2, N_T, incep_ker = [3,5,7,11], groups=8):
        super(Mid_Xnet, self).__init__()
        channel_in = hid_S * T
        channel_out = hid_S * T2
        channel_hid = hid_T
        self.T2 = T2
    ...
    def forward(self, x):
        ...
        y = z.reshape(B, self.T2, C, H, W)
        return y


class SimVP(LightningModule):
    def __init__(self, shape_in, shape_out, lr=1e-4, hid_S=16, hid_T=256, N_S=4, N_T=8, incep_ker=[3,5,7,11], groups=8, last_activation='sigmoid'):
        ...
        assert C2==C and H2==H and W2==W, 'Different input and output resolution is currently not supported'
        self.enc = Encoder(C, hid_S, N_S)
        self.hid = Mid_Xnet(hid_S, hid_T, T, T2, N_T, incep_ker, groups)
