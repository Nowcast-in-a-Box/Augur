...
# Handle timm imports safely
try:
    from timm.layers import DropPath, trunc_normal_
except ImportError:
    # Minimal fallback for DropPath and trunc_normal_ if timm is missing
...
class AFNONet(L.LightningModule):
    def __init__(
        ...
        # If params is provided, use it to override defaults
        if params:
            self.patch_size = (params.patch_size, params.patch_size)
            self.in_chans = params.N_in_channels
            self.out_chans = params.N_out_channels
    ...
    def configure_optimizers(self):
        lr = getattr(self.params, 'lr', 1e-4) if self.params else 1e-4
