...
class GenBlock(nn.Module):
    def __init__(self, fin, fout, evo_ic: int = 20, use_se=False, dilation=1, double_conv=False):
        """
        Generative Block using SPADE normalization.

        Args:
            fin (int): Number of input channels.
            fout (int): Number of output channels.
            evo_ic (int): Number of input channels for SPADE (from evolution result).
            use_se (bool): Whether to use Squeeze-and-Excitation. Defaults to False.
            dilation (int): Dilation factor for convolutions. Defaults to 1.
            double_conv (bool): Whether to use two convolutions in the path. Defaults to False.
        """
        super().__init__()
